...
defmodule AntikytheraAws.Auth.SourceType do
  use Croma.SubtypeOfAtom, values: [:instance], default: :instance
end

defmodule AntikytheraAws.Auth.Credentials do
  alias AntikytheraAws.Auth.SourceType
...
defmodule AntikytheraAws.Auth.InstanceProfileCredentials do
  ...
  defun credentials() :: nil | Credentials.t do
    case :ets.lookup(SystemCache.table_name(), @system_cache_key) do
      [{@system_cache_key, cached_credentials}] -> refresh_if_near_expiration(cached_credentials)
  ...
  defp store_credentials(%{"AccessKeyId" => access_key_id, "SecretAccessKey" => secret_key, "Expiration" => expiration_str, "Token" => token}) do
