...
defmodule AntikytheraAws.Auth.InstanceProfileCredentialsTest do
  ...
  test "should get and cache Credentials from instance metadata API (if not already cached)" do
    ...
    [{_key, %{credentials: cached_creds, expiration: expiration}}] =
      :ets.lookup(SystemCache.table_name(), @system_cache_key)
  ...
  test "should use cached Credentials if not expired" do
    # expires in 60mins
    time = Time.now() |> Time.shift_hours(1)

    active_credentials = %Creds{
      @valid_credentials
      | access_key_id: "not_expired",
        secret_access_key: "not_expired"
    }
  ...
  test "should refresh cached Credentials if expired" do
    # expired 10mins ago
    time = Time.now() |> Time.shift_minutes(-10)

    expired_credentials = %Creds{
      @valid_credentials
      | access_key_id: "expired",
        secret_access_key: "expired"
    }
  ...
  test "should refresh cached Credentials if expiring soon" do
    # expires in 2mins
    time = Time.now() |> Time.shift_minutes(2)

    expiring_credentials = %Creds{
      @valid_credentials
      | access_key_id: "expiring",
        secret_access_key: "expiring"
    }
  ...
  test "should retry getting Credentials on instance metadata API timeout" do
    redefine_and_error = fn ->
      # redefine meck on 1st call, so that it returns success on 2nd call
      creds_endpoint_expect(@creds_endpoint_success)
      {:error, :timeout}
    end
