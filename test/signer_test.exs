...
defmodule AntikytheraAws.Signer.V4Test do
  ...
  test "canonical_query_string/1 should normalize query string as per AWS specification" do
    [
      {[], ""},
      # Sorted with key, ascending order (value order does not matter)
      {[{"k", "v"}, {"K", "V"}, {"k", "a"}], "K=V&k=a&k=v"},
  ...
  test "canonical_headers_string/1 should normalize headers as per AWS specification" do
    [
      ...
      {%{"host" => "example.com", "x-amz-date" => "20160101T000000Z"},
       "host:example.com\nx-amz-date:20160101T000000Z\n"},
      {%{"host" => "example.com", "my-header" => " White   Spaces    "},
  ...
  test "signing_key/4 should derive signing key and credential scope" do
    expected_key =
      <<196, 175, 177, 204, 87, 113, 216, 113, 118, 58, 57, 62, 68, 183, 3, 87, 27, 85, 204, 40,
        66, 77, 26, 94, 134, 218, 110, 211, 193, 84, 164, 185>>

    expected_scope = "20150830/#{@example_region}/iam/aws4_request"

    assert V4.signing_key_with_cscope(@example_secret, "20150830T123600Z", @example_region, "iam") ==
             {expected_key, expected_scope}
  end
  ...
  test "prepare_headers/8 should produce signed headers" do
    expected_auth_value =
      "AWS4-HMAC-SHA256 Credential=#{@example_id}/20150830/#{@example_region}/iam/aws4_request, SignedHeaders=content-type;host;x-amz-date, Signature=5d672d79c15b13162d9279b0855cfba6789a8edb4c82c400e06b5924a6f2b5d7"
    ...
    result =
      V4.prepare_headers(@example_creds, @example_region, "iam", :get, "/", "", headers, params)
  ...
  test "prepare_headers/8 should correctly produce signed headers for test suite examples" do
    ...
    assert r1["authorization"] ==
             "AWS4-HMAC-SHA256 Credential=#{@example_id}/20110909/#{@example_region}/host/aws4_request, SignedHeaders=date;host, Signature=be7148d34ebccdc6423b19085378aa0bee970bdc61d144bd1a8c48c33079ab09"
    ...
    assert r2["authorization"] ==
             "AWS4-HMAC-SHA256 Credential=#{@example_id}/20110909/#{@example_region}/host/aws4_request, SignedHeaders=date;host, Signature=6fb359e9a05394cc7074e0feb42573a2601abc0c869a953e8c5c12e4e01f1a8c"
    ...
    assert V4.canonical_query_string(p3) ==
    ...
    assert r3["authorization"] ==
             "AWS4-HMAC-SHA256 Credential=#{@example_id}/20110909/#{@example_region}/host/aws4_request, SignedHeaders=date;host, Signature=28675d93ac1d686ab9988d6617661da4dffe7ba848a2285cb75eac6512e861f9"
    ...
    r4 =
      V4.prepare_headers(@example_creds, @example_region, "host", :post, "/", "foo=bar", h4, p4)

    assert r4["authorization"] ==
             "AWS4-HMAC-SHA256 Credential=#{@example_id}/20110909/#{@example_region}/host/aws4_request, SignedHeaders=content-type;date;host, Signature=b105eb10c6d318d2294de9d49dd8b031b55e3c3fe139f2e637da70511e9e7b71"
  end

  test "prepare_headers/8 should raise if 'host' header missing" do
    catch_error(
      V4.prepare_headers(@example_creds, @example_region, "iam", :get, "/", "", %{}, [])
    )
  end
